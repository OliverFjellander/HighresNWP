...
def confusion_mat(twod_radar,twod_nwp):
    ...
    for i in range(len(twod_radar)):
        ...
        TS[i]=a/(a+c+b)
        FPR[i]=b/(b+d)
    return cm_final,ETS,PSS,FBI,TPR,TS,FPR
...
def produce_zonalstat(data,lons,lats,files,datatype):
    ...
    for i in range(0,len(data)):
        if datatype=="NWP":    
            ...
            zs.append(zonal_statistics((tif_path,1),"C:/Users/olive/Desktop/Speciale/Dokumenter/Rain_observation_network/Rain_gauge_network/25gridnwp.shp",ignore_nodata=False,polygons_might_overlap=False))
        elif datatype=="radar":
